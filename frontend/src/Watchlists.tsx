...
import { addTicker, createWatchlist, deleteWatchlist, getMarketData, getSinceLastReview, getWatchlistCheckpoint, listWatchlists, markWatchlistReviewed, removeTicker, renameWatchlist, type Checkpoint, type MarketData, type SinceReview, type Watchlist } from './watchlistApi'
...
function SinceReviewPanel({ watchlistId }: { watchlistId: number }) {
  const [result, setResult] = useState<SinceReview | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')
  useEffect(() => { setLoading(true); getSinceLastReview(watchlistId).then(setResult).catch((reason) => setError(reason instanceof Error ? reason.message : 'Unable to load market changes.')).finally(() => setLoading(false)) }, [watchlistId])
  if (loading) return <section className="mp-review-analysis"><p className="mp-eyebrow">Since last review</p><p className="mp-muted">Loading deterministic analysis...</p></section>
  if (error) return <section className="mp-review-analysis"><p className="mp-eyebrow">Since last review</p><p className="mp-negative">{error}</p></section>
  if (!result || result.status === 'NO_CHECKPOINT') return <section className="mp-review-analysis"><p className="mp-eyebrow">Since last review</p><p className="mp-muted">Mark this watchlist as reviewed to establish a comparison checkpoint.</p></section>
  return <section className="mp-review-analysis"><div className="mp-review-analysis-head"><p className="mp-eyebrow">Since last review</p><span className="mp-muted">{result.stocks.length} stocks analyzed</span></div>{result.stocks.length ? result.stocks.map((stock) => <div className="mp-change-row" key={stock.ticker}><div><strong>{stock.ticker}</strong><small>{stock.warning || stock.status}</small></div>{stock.materialityScore === null ? <span className="mp-muted">{stock.warning || 'No comparable market data'}</span> : <><b className={`mp-classification ${stock.classification}`}>{stock.classification}</b><span>Score {stock.materialityScore.toFixed(0)}</span><span>Price {stock.price.percentageChange === null ? 'N/A' : `${stock.price.percentageChange.toFixed(2)}%`}</span><span>Volume {stock.volume.signal}</span><span>Volatility {stock.volatility.signal}</span><span>Sector {stock.relativeMovement.relativeMovement === null ? 'N/A' : `${stock.relativeMovement.relativeMovement.toFixed(2)}%`}</span><small>{stock.contributions.filter((item) => item.contribution > 0).map((item) => `${item.name}: ${item.contribution.toFixed(1)}`).join(' · ') || 'No material contribution'}</small></>}</div>) : <p className="mp-muted">No watchlist tickers are available for analysis.</p>}</section>
}

type WatchlistCardProps = { watchlist: Watchlist; onUpdate: (watchlist: Watchlist) => void; onDelete: (id: number) => void; onError: (message: string) => void }

function WatchlistCardBody({ watchlist, onUpdate, onDelete, onError }: WatchlistCardProps) {
...
function WatchlistCard(props: WatchlistCardProps) {
  return <><WatchlistCardBody {...props} /><SinceReviewPanel watchlistId={props.watchlist.id} /></>
}

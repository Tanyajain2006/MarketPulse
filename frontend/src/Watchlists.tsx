...
import { addTicker, createWatchlist, deleteWatchlist, listWatchlists, removeTicker, renameWatchlist, type Watchlist } from './watchlistApi'
...
function Sidebar({ user }: { user: { name: string; email: string } }) {
  return <aside className="mp-sidebar"><Logo /><nav className="mp-nav" aria-label="Workspace navigation"><p className="mp-nav-label">Workspace</p><button type="button" className="mp-nav-item"><span>Overview</span></button><button type="button" className="mp-nav-item is-active"><span><Icon name="eye" />Watchlists</span><i /></button><button type="button" className="mp-nav-item"><span>Preferences</span></button><div className="mp-nav-divider" /><p className="mp-nav-label">Tools</p><button type="button" className="mp-nav-item"><span>Research desk</span></button></nav><div className="mp-profile"><div className="mp-avatar">{user.name.trim().charAt(0).toUpperCase()}</div><div className="mp-profile-copy"><strong>{user.name}</strong><span>{user.email}</span></div><button type="button" className="mp-signout" title="Sign out">↗</button></div></aside>
}
...
function WatchlistCard({ watchlist, onUpdate, onDelete, onError }: { watchlist: Watchlist; onUpdate: (watchlist: Watchlist) => void; onDelete: (id: number) => void; onError: (message: string) => void }) {
  ...
  const [busy, setBusy] = useState<'rename' | 'delete' | 'ticker' | 'remove' | null>(null)
...
export default function Watchlists() {
  ...
  return <div className="mp-app"><Sidebar user={user!} /><div className="mp-main"><Header /><main className="mp-content"><div className="mp-intro"><p className="mp-eyebrow">Your universe</p><h1>Watchlists</h1><p>Keep each investment thesis close. MarketPulse will measure what changed since your last review.</p></div><form className="mp-create" onSubmit={create}><input value={name} maxLength={100} onChange={(event) => setName(event.target.value)} placeholder="Name a new watchlist" aria-label="Name a new watchlist" /><button type="submit" disabled={creating || !name.trim()}><Icon name="plus" />{creating ? 'Creating...' : 'Create'}</button></form>{error && <InlineMessage>{error}<button type="button" onClick={() => setError('')} aria-label="Dismiss error"><Icon name="close" /></button></InlineMessage>}{loading ? <div className="mp-grid" aria-label="Loading watchlists">{[1, 2].map((item) => <div className="mp-skeleton" key={item} />)}</div> : watchlists.length ? <div className="mp-grid">{watchlists.map((watchlist) => <WatchlistCard key={watchlist.id} watchlist={watchlist} onUpdate={update} onDelete={(id) => setWatchlists((current) => current.filter((item) => item.id !== id))} onError={setError} />)}</div> : <section className="mp-empty"><span className="mp-card-icon"><Icon name="chart" /></span><h2>Your universe is empty</h2><p>Create a watchlist above to keep an investment thesis close.</p></section>}</main></div></div>
}